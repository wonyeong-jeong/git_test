...
import { buildCumulativeTimeline, mergeCostBasisAndSnapshots } from './assetGrowth'
...
describe('buildCumulativeTimeline', () => {
  it('날짜순으로 누적한다', () => {
    ...
    expect(timeline).toEqual([
      { date: '2026-01-01', cumulativeValue: 1_000_000 },
      { date: '2026-02-01', cumulativeValue: 1_500_000 },
      { date: '2026-03-01', cumulativeValue: 1_300_000 }
    ])
  })
  ...
  it('같은 날짜는 하나의 포인트로 합친다', () => {
    ...
    expect(timeline).toEqual([{ date: '2026-01-01', cumulativeValue: 1_200_000 }])
  })

  it('빈 배열이면 빈 결과', () => {
    expect(buildCumulativeTimeline([])).toEqual([])
  })

  it('수량 누적에도 그대로 쓸 수 있다(매도 시 보유수량 감소)', () => {
    const timeline = buildCumulativeTimeline([
      { date: '2026-01-01', amount: 10 },
      { date: '2026-02-01', amount: -3 }
    ])
    expect(timeline).toEqual([
      { date: '2026-01-01', cumulativeValue: 10 },
      { date: '2026-02-01', cumulativeValue: 7 }
    ])
  })
})
...
describe('mergeCostBasisAndSnapshots', () => {
  it('원금은 계단식으로 이어지고 스냅샷은 해당 날짜에만 값이 있다', () => {
    const costBasis = [
      { date: '2026-01-01', cumulativeValue: 1_000_000 },
      { date: '2026-01-15', cumulativeValue: 1_500_000 }
    ]
    const snapshots = [

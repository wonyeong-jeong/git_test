 ...
 *   1) 누적 투입원금(cost basis): 보유종목 등록일 + 매수/매도 기록 날짜로부터 계산되는,
 *      "지금까지 얼마를 넣었는가"의 실제 히스토리. 시세와 무관하게 정확하다.
...
/** 날짜별 원금 증감을 오래된 순으로 누적한다. 같은 날짜의 여러 건은 하나의 포인트로 합쳐진다 */
export function buildCostBasisTimeline(deltas: CostDelta[]): CostBasisPoint[] {
  ...
  return sortedDates.map((date) => {
    ...
    return { date, cumulativeCost: running }
  })
}
 ...
 */
export function mergeCostBasisAndSnapshots(costBasis: CostBasisPoint[], snapshots: SnapshotPoint[]): GrowthChartPoint[] {
  ...
  return allDates.map((date) => {
    while (costIndex < sortedCost.length && sortedCost[costIndex].date <= date) {
      lastCost = sortedCost[costIndex].cumulativeCost
      costIndex++
    }
